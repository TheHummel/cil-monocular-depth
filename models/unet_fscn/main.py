...
from models.dpt_hybrid_midas.fscn.CustomFSCNFusionStage import UnetFSCNFusionStage
...
class SimpleUNet(nn.Module):
    def __init__(self, output_size):
        ...
        self.bottom = UNetBlock(256, 256)

        # channel projection layers for decoder features
        self.proj_to_256 = nn.Conv2d(256, 256, kernel_size=1)
        self.proj_to_128 = nn.Conv2d(256, 128, kernel_size=1)
        self.proj_to_64 = nn.Conv2d(256, 64, kernel_size=1)

        # fusion stage
        self.fusion_stage = UnetFSCNFusionStage(
            encoder_channels=[32, 64, 128],
            decoder_channels_list=[256, 128, 64],
            out_channels_list=[128, 64, 32]
        )

        # decoder blocks
        self.dec3 = UNetBlock(128, 128)
        self.dec2 = UNetBlock(64, 64)
    ...
    def forward(self, x):
        ...
        # decoder with fusion
        encoder_features = [enc1, enc2, enc3]
        decoder_features = [
            self.proj_to_256(x),  # 256 channels
            self.proj_to_128(nn.functional.interpolate(x, size=enc2.shape[2:], mode="bilinear", align_corners=True)),
            self.proj_to_64(nn.functional.interpolate(x, size=enc1.shape[2:], mode="bilinear", align_corners=True))
        ]
        fused_features = self.fusion_stage(decoder_features, encoder_features)
